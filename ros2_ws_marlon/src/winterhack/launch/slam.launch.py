...
def launch_setup(context):
    ...
    winterhack_package_path = get_package_share_directory('winterhack')

    slam_params = RewrittenYaml(
        source_file=os.path.join(winterhack_package_path, 'config/slam.yaml'),
        param_rewrites={
    ...
    rviz_node = Node(
            ...
            executable='rviz2',
            name='rviz2_slam',
            arguments=['-d', os.path.join(winterhack_package_path, 'rviz/slam.rviz')],
    ...
    return [
        ...
        scan_topic_arg,
        sync_node,
        # rviz_node  # 禁用SLAM RViz，避免与导航RViz冲突
    ]
