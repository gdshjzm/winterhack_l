...
DEFAULT_PRIORITY = ["RED", "BLUE", "YELLOW"]
NUM_OF_TARGETS = 3
...
class MissionRunner(Node):
    ...
    def __init__(self) -> None:
        ...
        self.seenColors = {} # Dictionary to hold seen colors and their states
        self._cb_group = ReentrantCallbackGroup()
        self._timer_cb_group = MutuallyExclusiveCallbackGroup()

        self.declare_parameter("home_xy", [0.0, 0.0])
        self.declare_parameter("target_pick_count", NUM_OF_TARGETS)
        ...
        # Added code - tracking position of robot
    ...
    # Added code - Gets current coordinates of robot using SLAM TF2
    def get_robot_xy(self) -> Optional[Tuple[float, float]]:
        """Get current robot (x, y) in map frame."""
        try:
            transform = self.tf_buffer.lookup_transform(
                "map",              # target frame (slam creates this)
    ...
    # Gets current angle of robot using SLAM
    def get_robot_yaw(self) -> Optional[float]:
        """Get current robot yaw (rad) in map frame."""
        try:
            transform = self.tf_buffer.lookup_transform(
                "map",
                "base_footprint",
            ...
            # Manual conversion from quaternion to yaw
            yaw = math.atan2(2.0 * (qw * qz + qx * qy), 1.0 - 2.0 * (qy * qy + qz * qz))
            return yaw
        except (LookupException, ConnectivityException, ExtrapolationException):
            return None

    def _approximate_position(self, detected_distance: float) -> Optional[Tuple[float, float]]:
        """Approximate position of detected color based on robot's current position and yaw."""
        position = self.get_robot_xy()
        yaw = self.get_robot_yaw()
        if position is None or yaw is None:
            return None
        if detected_distance is None:
            detected_distance = 0.0
        approx_x = position[0] + detected_distance * math.cos(yaw)
        approx_y = position[1] + detected_distance * math.sin(yaw)
        return (approx_x, approx_y)

    def _on_detection(self, msg: String) -> None:
        if self._state != MissionPhase.PATROL_SEARCH and self._state != MissionPhase.KNOWN_POSITION:
            return
        detected = self._extract_detected_colour_distance(msg)
        if detected is None or detected[0] is None:
            return
        if detected[0] != DEFAULT_PRIORITY[self.picked_total]: 
            # Approximates position of non-priority colors and stores in seenColors
            position = self._approximate_position(detected[1])
            self.seenColors[detected[0]] = position
            self.get_logger().info(f"Detected non-priority color: {detected[0]} at position {position}, adding to dictionary.")
            return

        if detected[0] in self.detected_colours: # Redundant with priority check
            return
        
        self.detected_colour = detected[0]
        self.detected_colours.add(detected[0])
        self.get_logger().info(
            f"Qualifying detection: {self.detected_colour}; stopping patrol."
        )
        self._set_state(MissionPhase.STOP_SEARCH)

    def _extract_detected_colour_distance(self, msg: String) -> Optional[Tuple[str, float]]:
        # Now also returns distance of colour from block for better approximation
        ...
        if isinstance(detections, list):
            for det in detections:
                ...
                distance = det.get("estimated_distance_m")
                if color:
    ...
    def _tick(self) -> None:
        if self._state == MissionPhase.IDLE: #No mission running
            return
        if self._state == MissionPhase.INIT: #Starts mission
            self.picked_total = 0
            self.detected_colour = None
            self.detected_colours.clear()
            self._set_state(MissionPhase.PATROL_SEARCH)
            return
        if self._state == MissionPhase.KNOWN_POSITION: # Goes to known position of current priority block if in seenColors
            self._tick_known_position()
            return
        if self._state == MissionPhase.PATROL_SEARCH: #Goes around arena looking for blocks
            self._tick_patrol()
            return
        if self._state == MissionPhase.STOP_SEARCH: #Stops patrol when a block is found
            self._tick_stop_search()
            return
        if self._state == MissionPhase.PICK: #picks up block
            self._tick_pick()
            return
        if self._state == MissionPhase.LOCATE: #orients to pick up block
            self._tick_locate()
            return
        if self._state == MissionPhase.RETRIEVE_HOME: #takes block home
            self._tick_retrieve_home()
            return
        if self._state == MissionPhase.DROP: #drops block
            self._tick_drop()

    def _tick_known_position(self) -> None:
        # Set global targets to known position of the current priority block
        # If fails, revert to regular patrol
        if self._temp_targets is None:
            self._temp_targets = self._global_targets.copy()
        self._global_targets = [self.seenColors[DEFAULT_PRIORITY[self.picked_total]]]

        # Rest of code is same as _tick_patrol, except instead of restarting patrol, set _global_targets back to _temp_targets
        ...
        if self._follow_result_future is not None and self._follow_result_future.done():
            ...
            self.get_logger().info(
                f"Patrol finished with status {status}; reverting to regular patrol."
            )
            # Restore original global targets
            self.get_logger().info(f"KNOWN_POSITION targets={self._temp_targets}")
            self._global_targets = self._temp_targets
            self.get_logger().info(f"Restored PATROL_SEARCH targets={self._global_targets}")
            self._state = MissionPhase.PATROL_SEARCH
            self._temp_targets = None
    ...
    def _tick_locate(self) -> None:
        ...
        if not self._locate_client.server_is_ready():
            self.get_logger().warning("Locate server not ready; retrying...")
            return
    ...
    def _tick_drop(self) -> None:
        if not self._enable_drop:
            ...
            if self.picked_total < self._target_pick_count:
                if DEFAULT_PRIORITY[self.picked_total] in self.seenColors:
                    self._set_state(MissionPhase.KNOWN_POSITION)
                else:
                    self._set_state(MissionPhase.PATROL_SEARCH)
            else:
    ...
    def _send_locate_goal(self) -> None:
        if not self._locate_client.wait_for_server(timeout_sec=2.0):
            self.get_logger().warning(
    ...
    def _update_follow_goal(self) -> None:
        if self._follow_goal_future is not None and self._follow_goal_future.done():
            goal_handle = self._follow_goal_future.result() #Some issue with goal_handle here
            self._follow_goal_future = None
            if goal_handle is None or not goal_handle.accepted: #Issue is with goal_handle.accepted
