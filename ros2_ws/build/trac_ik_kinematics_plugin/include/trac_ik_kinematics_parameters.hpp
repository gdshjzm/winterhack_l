#pragma message("#include \"trac_ik_kinematics_parameters.hpp\" is deprecated. Use #include <trac_ik_kinematics_plugin/trac_ik_kinematics_parameters.hpp> instead.")
...
[[nodiscard]] auto to_parameter_value(rsl::StaticVector<T, capacity> const& value) {
    ...
    struct Params {
        ...
        std::string solve_type = "Distance";
  ...
  public:
    ...
    : ParamListener(parameters_interface, rclcpp::get_logger("trac_ik_kinematics"), prefix) {
    ...
    rcl_interfaces::msg::SetParametersResult update(const std::vector<rclcpp::Parameter> &parameters) {
      ...
      for (const auto &param: parameters) {
        ...
        if (param.get_name() == (prefix_ + "solve_type")) {
            if(auto validation_result = one_of<std::string>(param, {"Speed", "Distance", "Manipulation1", "Manipulation2"});
              !validation_result) {
                return rsl::to_parameter_result_msg(validation_result);
            }
            updated_params.solve_type = param.as_string();
            RCLCPP_DEBUG_STREAM(logger_, param.get_name() << ": " << param.get_type_name() << " = " << param.value_to_string());
        }
      }
    ...
    void declare_params(){
      ...
      if (!parameters_interface_->has_parameter(prefix_ + "solve_type")) {
          rcl_interfaces::msg::ParameterDescriptor descriptor;
          descriptor.description = "Solve Type. Options: Speed, Distance, Manipulation1, Manipulation2";
          descriptor.read_only = false;
          auto parameter = to_parameter_value(updated_params.solve_type);
          parameters_interface_->declare_parameter(prefix_ + "solve_type", parameter, descriptor);
      }
      ...
      RCLCPP_DEBUG_STREAM(logger_, param.get_name() << ": " << param.get_type_name() << " = " << param.value_to_string());
      updated_params.position_only_ik = param.as_bool();
      param = parameters_interface_->get_parameter(prefix_ + "solve_type");
      RCLCPP_DEBUG_STREAM(logger_, param.get_name() << ": " << param.get_type_name() << " = " << param.value_to_string());
      if(auto validation_result = one_of<std::string>(param, {"Speed", "Distance", "Manipulation1", "Manipulation2"});
        !validation_result) {
          throw rclcpp::exceptions::InvalidParameterValueException(fmt::format("Invalid value set during initialization for parameter 'solve_type': {}", validation_result.error()));
      }
      updated_params.solve_type = param.as_string();
